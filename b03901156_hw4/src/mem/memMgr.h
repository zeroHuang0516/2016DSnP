...
public:
   ...
   void reset(size_t b = 0) {
      ...
      while(_activeBlock->_nextBlock!=NULL){
         MemBlock<T>* newMemBlock = _activeBlock->_nextBlock;
         delete _activeBlock;
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
     // n=getArraySize(toSizeT(n));
      size_t m = getArraySize(toSizeT(n));
      // TODO ..ok
      MemRecycleList<T>* current = &_recycleList[m%R_SIZE] ;
      MemRecycleList<T>* last = NULL;
      while(current!=NULL){
          if(current->_arrSize == m ){
              return current;
          }
          last=current;
      ...
      current = new MemRecycleList<T>;
      current->_arrSize= m ;
      current->_nextList=NULL;
      last->_nextList = current;
      return current;
   }
   ...
   T* getMem(size_t t) {
      ...
      if(this->getMemRecycleList(size)->_first!=NULL){
          ret = this->getMemRecycleList(size)->popFront();
          #ifdef MEM_DEBUG
          size_t n = getArraySize(size);
          cout << "Recycled from _recycleList[" << n << "]..." << ret << endl;
          #endif // MEM_DEBUG
      }
      ...
      //    #endif // MEM_DEBUG
          ...
          if(!_activeBlock->getMem(size,ret)){
              size_t remain=_activeBlock->getRemainSize();
              if(remain>=S){
                  _activeBlock->getMem(downtoSizeT(remain),ret);
                  getMemRecycleList(remain)->pushFront(ret);
                  #ifdef MEM_DEBUG
                  size_t rn = getMemRecycleList(remain)->_arrSize;
                  cout << "Recycling " << ret << " to _recycleList[" << rn << "]\n";
                  #endif // MEM_DEBUG
              }
              #ifdef MEM_DEBUG
   ...
   size_t getNumBlocks() const {
      // TODO..ok
      MemBlock<T>* tmpActiveBlock = _activeBlock;
      size_t counter=0;
      do{
         tmpActiveBlock=tmpActiveBlock->getNextBlock();
         counter++;
      }while(tmpActiveBlock!=NULL);
      return counter;
   }
