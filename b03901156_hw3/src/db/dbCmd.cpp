...
DBAppendCmd::exec(const string& option)
{
  // OK
   ...
   while (token.size()) {
      ...
      pos= myStrGetTok(option, token, pos);
   }
   
   for(size_t u=1;u<options.size();u++){
       if(!myStr2Int(options[u],num) && options[u]!="-" ){
         cerr<<"Error: "<<"\""<<options[u]<<"\""<<" is not an integer!!"<<endl;
         flag=false;
          break;
       }
   }
     ...
     if(options.size()-1<=dbtbl.nCols()){
       for(size_t i=1;i<options.size();i++){
               if(options[i]=="-"){
                   ...
                   r.addData(atoi(options[i].c_str()));
               }
       }
       for(size_t i=options.size()-1;i<dbtbl.nCols();i++){
           r.addData(INT_MAX);
       }
     }
     else{
     ...
     if(options.size()-1<=dbtbl.nRows()){
      for(size_t i=1;i<options.size();i++){
         if(options[i]=="-"){
                   ...
                   c.push_back(atoi(options[i].c_str()));
               }
       }
       for(size_t i=options.size()-1;i<dbtbl.nRows();i++){
           c.push_back(INT_MAX);
       }
     }
     else{
...
{  
   ...
   string firstToken;
   int num;
   myStrGetTok(option, firstToken);
   if( myStr2Int(firstToken, num)!=false ){
       CmdExec::lexOptions(option,options, 2 );
       if( !(atoi(options[0].c_str())>=0 && atoi(options[0].c_str())<dbtbl.nRows()) ){
           cerr<<"Error: Row index "<<options[0]<<" is out of range!!"<<endl;
       }
       else if( !(atoi(options[1].c_str())>=0 && atoi(options[1].c_str())<dbtbl.nCols()) ){
           cerr<<"Error: Column index "<<options[1]<<" is out of range!!"<<endl;
       }
       else{
           if(dbtbl.getData( atoi(options[0].c_str()),atoi(options[1].c_str()) )==INT_MAX){
               cout<<"null"<<endl;
           }
           else{
               cout<<dbtbl.getData( atoi(options[0].c_str()),atoi(options[1].c_str()) )<<endl;
           }
       } 
   }
   else if(myStrNCmp("-Row", firstToken,2 ) == 0){
       CmdExec::lexOptions(option, options, 2 );
       if( !(atoi(options[1].c_str())>=0 && atoi(options[1].c_str())<dbtbl.nRows()) ){
           cerr<<"Error: Row index "<<options[1]<<" is out of range!!"<<endl;
       }
       else{
           cout<<dbtbl[atoi(options[1].c_str())];
       }
   }
   else if(myStrNCmp("-Column", firstToken,2 ) == 0){
       CmdExec::lexOptions(option, options, 2 );
       if( !(atoi(options[1].c_str())>=0 && atoi(options[1].c_str())<dbtbl.nCols()) ){
           cerr<<"Error: Column index "<<options[1]<<" is out of range!!"<<endl;
       }
       else{
           dbtbl.printCol(atoi(options[1].c_str()));
       }
   }
   else if(myStrNCmp("-Table", firstToken,2 ) == 0){
       cout<<dbtbl;
   }
   else if(myStrNCmp("-Summary", firstToken,2 ) == 0){
       dbtbl.printSummary();
   }
